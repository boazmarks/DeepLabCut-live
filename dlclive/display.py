...
class Display(object):
    ...
    def display_frame(self, frame, axis=None):
        """
        ...
        if axis is not None:
            ...
            if self.window is None:
                self.set_display(im_size)

            img = Image.fromarray(frame)
            draw = ImageDraw.Draw(img)
            try:
                draw.line([axis[0], axis[1]], fill = "blue", width = 5)
                draw.line([axis[0], axis[2]], fill = "green", width = 5)
                draw.line([axis[0], axis[3]], fill = "red", width = 5)
